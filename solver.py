...
def solve(r, goalState):
    ...
    while( not checkPath(queue[0]) and len(queue)!=0 ):
        ...
        longersRemoved = removeLongers(loopsRemoved)    #REMOVE UNNECESSAIRLY LONGER ONES FROM THEM
        addToFront( queue, longersRemoved )     #ADD THESE NEW PATHS TO THE QUEUE       
        queue = orderPaths(queue)               #ORDER THE QUEUE WITH LOWER COST ONES IN FRONT        
